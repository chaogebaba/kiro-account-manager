...
import { Github, Loader, CheckCircle, Key } from 'lucide-react'
...
function Login({ onLogin, onCancel }) {
  ...
  const [tokenForm, setTokenForm] = useState({ accessToken: '', refreshToken: '', provider: 'Google' })
  ...
  const handleTokenSubmit = async () => {
    if (!tokenForm.accessToken || !tokenForm.refreshToken) return
    setLoading('token')
    setError('')
    try {
      // 调用后端，自动获取邮箱和配额
        ...
        {showTokenInput ? (
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                value={tokenForm.provider}
                onChange={(e) => setTokenForm({ ...tokenForm, provider: e.target.value })}
                className="w-full px-3 py-2 bg-[#2a2a3c] border border-[#3a3a4c] rounded-lg text-white text-sm"
              >
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1">Access Token (aoa开头)</label>
              <textarea
                value={tokenForm.accessToken}
                onChange={(e) => setTokenForm({ ...tokenForm, accessToken: e.target.value })}
                placeholder="aoaAAAAA..."
                className="w-full px-3 py-2 bg-[#2a2a3c] border border-[#3a3a4c] rounded-lg text-white text-sm h-20 resize-none font-mono"
              />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Refresh Token (aor开头)</label>
              <textarea
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={loading === 'token' || !tokenForm.accessToken || !tokenForm.refreshToken}
                className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium disabled:opacity-50"
              >
                {loading === 'token' ? '验证中...' : '添加账号'}
              </button>
            </div>
          </div>
        ) : (
          ...
          <div className="space-y-3">
            ...
            <div className="relative my-4">
              <div className="absolute inset-0 flex items-center">
                <div className="w-full border-t border-[#3a3a4c]"></div>
              </div>
              <div className="relative flex justify-center text-xs">
                <span className="px-2 bg-[#1a1a24] text-gray-500">或</span>
              </div>
            </div>

            <button
              onClick={() => setShowTokenInput(true)}
              className="w-full flex items-center gap-3 px-4 py-3 bg-[#2a2a3c] hover:bg-[#353548] rounded-lg border border-[#3a3a4c] transition-colors"
            >
              <Key size={20} className="text-gray-400" />
              <span className="text-white font-medium">手动输入 Token</span>
            </button>
          </div>
        )}
