...
import { testRouteConfig } from '../../../api/gatewayApi'
import { buildGatewayPayload, type GatewayConfig } from './gatewayPageState'
...
export function RouteTestDialog({ open, onOpenChange, config }: RouteTestDialogProps) {
  ...
  const handleTest = async () => {
    setIsTesting(true)
    try {
      const testResult = await testRouteConfig(buildGatewayPayload(config))
