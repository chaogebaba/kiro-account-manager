...
const zh = {
  ...
  addAccount: {
    ...
    social: {
      ...
      success: '授权成功！',
      failed: '授权失败',
      browserHint: '复制下方登录地址，在你自己选择的浏览器中打开并完成登录，完成后本窗口会自动返回。',
      loginUrl: '登录地址',
      copyUrl: '复制登录链接',
      copied: '已复制到剪贴板',
      expiresIn: '{seconds} 秒后过期',
      remoteTitle: '浏览器在另一台机器？',
      remoteHint: '把浏览器最终停留的完整回调地址粘进来，形如 http://127.0.0.1:3128/oauth/callback?code=...&state=...',
      remotePlaceholder: 'http://127.0.0.1:3128/oauth/callback?code=...&state=...',
      remoteSubmit: '完成登录',
      syncAfterLogin: '登录后设为当前账号（同步到 kiro-cli / Kiro IDE）'
    },
    batch: {
      title: '批量导入',
  ...
  errors: {
    ...
    SOCIAL_LOGIN_PORTS_BUSY: '本地回调端口全被占用，请关掉占用 3128 / 4649 / 6588 / 8008 / 9091 的程序后重试',
    SOCIAL_LOGIN_EXPIRED: '登录已超时（10 分钟），请重新发起',
    SOCIAL_LOGIN_STATE_MISMATCH: 'OAuth state 不匹配，回调已被拒绝，请重新发起登录',
    SOCIAL_LOGIN_EXCHANGE_FAILED: '授权码换 Token 失败',
    SOCIAL_LOGIN_NO_REFRESH_TOKEN: '登录未返回 refreshToken，无法维持登录态，请重试',
    SOCIAL_LOGIN_CANCELLED: '登录已取消',
